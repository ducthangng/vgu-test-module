...
func (q *Querier) QueryTestResultIndexScore(ctx context.Context, TestClassID int, UserID int, DateCreate time.Time, Flag int) ([]entity.TestResult, error) {
	...
	switch Flag {
	case 1:
		rows, err = q.DB.QueryContext(ctx, "SELECT id, test_class_id, user_id, datecreated, score, comment, result_note FROM testresults WHERE test_class_id = ? and active = ?", TestClassID, 1)
	case 2:
		rows, err = q.DB.QueryContext(ctx, "SELECT id, test_class_id, user_id, datecreated, score, comment, result_note FROM testresults WHERE user_id = ? and active = ?", UserID, 1)
	case 3:
		rows, err = q.DB.QueryContext(ctx, "SELECT id, test_class_id, user_id, datecreated, score, comment, result_note FROM testresults WHERE datecreated = ? and active = ?", DateCreate, 1)
	case 4:
		rows, err = q.DB.QueryContext(ctx, "SELECT id, test_class_id, user_id, datecreated, score, comment, result_note FROM testresults WHERE test_class_id = ? AND user_id = ? AND active = ?", TestClassID, UserID, 1)
	case 5:
		rows, err = q.DB.QueryContext(ctx, "SELECT id, test_class_id, user_id, datecreated, score, comment, result_note FROM testresults WHERE test_class_id = ? AND datecreated = ? AND active = ?", TestClassID, DateCreate, 1)
	case 6:
		rows, err = q.DB.QueryContext(ctx, "SELECT id, test_class_id, user_id, datecreated, score, comment, result_note FROM testresults WHERE user_id = ? AND datecreated = ? AND active = ?", UserID, DateCreate, 1)
	case 7:
		rows, err = q.DB.QueryContext(ctx, "SELECT id, test_class_id, user_id, datecreated, score, comment, result_note FROM testresults WHERE test_class_id = ? AND user_id = ? AND datecreated = ? AND active = ?", TestClassID, UserID, DateCreate, 1)
	default:
